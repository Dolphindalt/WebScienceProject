...
class Threads extends ControllerBase {
    ...
    public function createThreadTask() {
        ...
        if (!PostTimerService::testPostTimer(OP_CREATE_POST_OR_THREAD, 30)) {
            http_response_code(400);
            $this->showErrorOnBoardCatalog($board_dir, "Wait 30 seconds before registering again.");
            die();
        }
    ...
    public function createPostOnThreadTask() {
        ...
        if (!PostTimerService::testPostTimer(OP_CREATE_POST_OR_THREAD, 30)) {
            http_response_code(400);
            $this->showErrorOnThreadPage($board_dir, $thread_id, "Wait 30 seconds before posting again.");
            die();
        }
